...
but applied with a Kustomize base + per-cell overlay.
...
base/                # shared postgres + demo app + ingress + netpol manifests
cell-a/              # kustomize overlay: applies base + labels resources cell: a
cell-b/              # kustomize overlay: applies base + labels resources cell: b
...
Docker, kind, kubectl. nginx optional (`router.conf`).
...
```bash
kubectl apply -k cell-a --context kind-cell-a
kubectl apply -k cell-b --context kind-cell-b
```
